...
Both classes contain outliers and bimodal distributions. Deceases patients have positive skew in the density plot. 
...
The box plot of Death_Event and Platelets shows many outliers in the number of Platelets for patients who survived and two outliers in patients who died. Both classes loosely follow a normal distribution. 
...
As was the case for Platelets, CPK_Level, and Ejection_Fraction, outliers are observed for Serum Creatinine levels in patients who survived/died. The density plot is skewed positively for both classes, but is more apparent for deceased patients.
...
From the plots above, patients having a shorter follow up time died, while patients with a longer follow up time survived. The distribution of deaths and survivals are different, deaths are unimodal on the density plot and survivors are bimodal.
...
as_tibble() can be used to display the accuracy that was achieved with the optimal model. This can be very useful when there are multiple hyperparameters to tune and the output is long. 
...
The final test of time will be to test each models performance on the data we tucked away while the models were being trained. If the model gets a better accuracy on the testing data than the training data, then it was able to generalize well and overfitting was likely avoided. 
...
The Decision Tree was able to achieve an accuracy of 81% and precision of 71% on the testing data. During training, the model reported an accuracy of 69%.
...
Using Random Forests an accuracy of 82% and precision of 84% was accomplished. The model only performed with 74% accuracy on the training data.
...
So far the Support Vector Machine has performed the worst, with an accuracy of 72% and precision of 64%. During training, the model had 74% accuracy on the training data. The model overfit with the training data and performed worse when testing.  
...
Gradient Boosting performed well, an accuracy of 80% and precision of 79% was recorded. During training, the model performed with 74% accuracy. 

Time to revisit feature importance. How did GBM rank the most important features in the dataset? 

```{r, message=FALSE}
gbm_sum = summary(gbm, plotit=FALSE)
```
```{r}
gbm_sum
```

The results from the Mann-Whitney U test listed Creatinine, Ejection_Fraction, Age and Sodium as the most important features (disregarding Time) in order of importance. Interesting enough, the GB model listed Creatinine, Ejection_Fraction, Age and Sodium in the same order as the ranking in the Mann-Whitney U test. 
